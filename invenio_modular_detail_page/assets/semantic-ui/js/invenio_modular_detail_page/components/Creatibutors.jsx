...
const CreatibutorIcons = ({ creatibutor, schemeInfo, textLabel = false }) => {
      ...
      {!!ids
        ? ids.map(({ scheme, identifier }) => (
            <a
              ...
              className={`no-text-decoration ${scheme}-icon-link identifier-icon-link`}
              key={`${scheme}-${identifier}`}
              aria-label={`${creatibutor.person_or_org.name}'s ${
                schemeInfo[scheme][0]
              } ${i18next.t("profile")}`}
            >
              {scheme.toLowerCase() === "email" ? (
                <Icon
                  name="mail outline"
                ...
                <img
                  className={`mr-5 inline-id-icon ${scheme}`}
              ...
              {!!textLabel ? (
                <span className="mr-5">{`${schemeInfo[scheme][0]} ${i18next.t("profile")}`}</span>
              ) : null}
            </a>
          ))
        : ""}
      {!!ids && creatibutor.person_or_org.type == "organizational" ? <Icon name="group" /> : ""}
    </>
  );
};
...
const Creatibutor = ({
  ...
  if (show_affiliations && creatibutor.affiliations) {
    extra_props["data-tooltip"] = creatibutor.affiliations.map((a) => a[1]).join("; ");
  }
  return (
      ...
      <Card.Content className="pb-5">
        ...
        <Card.Description className="mt-0">
          {!!creator.affiliations &&
            show_affiliations &&
            creator.affiliations.map((a) => <small className="creator-affiliation">{a[1]}</small>)}
        </Card.Description>
      </Card.Content>
      <Card.Content extra className="mt-0 pt-5">
        <CreatibutorIcons creatibutor={creator} schemeInfo={schemeInfo} textLabel={true} />
      </Card.Content>
    </Card>
  );
};
...
const Creatibutors = ({
  ...
  const primaryCreatibutors = buildCreatibutorsList(creators?.creators || []);
  const secondaryCreatibutors = buildCreatibutorsList(contributors?.contributors || []);
  const schemeInfo = makeSchemeInfo(
    iconsGnd,
    iconsKcUsername,
    iconsIsni,
